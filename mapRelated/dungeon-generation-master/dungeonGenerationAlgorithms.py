...
class RoomAddition:
	...
	def addShortcuts(self,mapWidth,mapHeight):
		'''
		I use tcodpy's built in pathfinding here, since I'm
		already using tcodpy for the iu. At the moment,
		...
		#initialize the tcodpy map
		tcodMap = tcod.map_new(mapWidth,mapHeight)
		self.recomputePathMap(mapWidth,mapHeight,tcodMap)
				...
				for y in xrange(-1,2):
					if x != 0 or y != 0: # Exclude the center tile
						...
						# run pathfinding algorithm between the two points
							#back to the tcodpy nonesense
							pathMap = tcod.path_new_using_map(tcodMap)
	...
	def recomputePathMap(self,mapWidth,mapHeight,tcodMap):
		for x in xrange(mapWidth):
			for y in xrange(mapHeight):
				if self.level[x][y] == 1:
					tcod.map_set_properties(tcodMap,x,y,False,False)
				else:
					tcod.map_set_properties(tcodMap,x,y,True,True)
