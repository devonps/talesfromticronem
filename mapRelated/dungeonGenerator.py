##################################################################
#                                                                #
# Procedural Dungeon Generator v1.5                              #
#                                                                #
# By Jay (Battery)                                               #
#                                                                #
...
#                                                                #
# Updated v1.1 to v1.5 by GK101  11/28/2018                      #
# Added features:                                                #
#   Constants:                                                   #
#       OVERLAY                                                  #
#       OVERLAY_ARTIFACT                                         #
...
#   methods:                                                     #
#       placeRandomOverlays                                      #
#       placeEnteranceExitOverlay                                #
#       _loadOverlay                                             #
#       _removeMatchTiles                                        #
#                                                                #
#                                                                #
# Updated v1.5 to v1.6 by GK101  12/10/2018                      #
#   methods:                                                     #
#       findHostableTiles                                        #
#       removeUnconnectedAreas                                   #
#       mergeUnconnectedAreas                                    #
#   Optimized:                                                   #
#       _removeMatchTiles                                        #            
##################################################################
...
class dungeonGenerator:
	...
	def removeUnconnectedAreas(self):
		"""
		    Added to just make it easier for end user to remove "islands"
		    from generated map
		"""
	...
	def mergeUnconnectedAreas(self):
		"""
		    Added to just make it easier for end user to remove "islands"
		    from generated map and join areas
		"""
		unconnected = self.findUnconnectedAreas()
		self.joinUnconnectedAreas(unconnected)
	...
	def findUnconnectedAreas(self):
		...
		Updated by GK101 12/10-2018 to resolve non-square map errors 
		Args:
		    none
	...
	def _loadOverlay(self, x, y, overlay):
		key = "{0},{1}".format(x,y)
		if key not in self.overlays:
			self.overlays[key] = (x,y,{overlay:1})
		else:
			if overlay not in self.overlays[key][2]:
				self.overlays[key][2][overlay] = 1
			else:
				self.overlays[key][2][overlay] += 1
		return

	def _removeMatchTiles(self, modify, check):
		modify = [x for x in modify if x not in check]
		return

	def findHostableTiles(self, allowDeadends=True, allowCorridor=True, allowCaves=True):
		"""
		Searches all tiles to find matches to defined tiles

		Args:
		    <boolean> allowDeadends: markers allowed in deadends
		    <boolean> allowCorridor: markers allowed in any corridor tile
		    <boolean> allowRooms: markers allowed in rooms
		    <boolean> allowCave: markers allowed on cave tiles
		...
		allowedTiles = [FLOOR]
		if allowCorridor:
			allowedTiles.append(CORRIDOR)
		if allowCaves:
			allowedTiles.append(CAVE)

		for x in range(self.width):
			for y in range(self.height):
				if self.grid[x][y] in allowedTiles:
					self.hostabletiles.append((x,y,self.grid[x][y]))

		if not allowDeadends:
			self._removeMatchTiles(src, self.deadends)

	def placeEnteranceExitOverlay(self, allowOthers=True, allowDeadends=True, allowCorridor=False, allowRooms=False, allowCaves=True):
		"""
		Places Enterance/Exit markers
		This should get called prior to anyother overlay calls

		Args:
		    <boolean> allowOthers: allows other overlays to occupy the same space
		    <boolean> allowDeadends: markers allowed in deadends
		    <boolean> allowCorridor: markers allowed in any corridor tile
		    <boolean> allowRooms: markers allowed in rooms
		    <boolean> allowCave: markers allowed on cave tiles
		...
		Returns:
		    none
		"""
		# exit due to bad options
		if not allowDeadends and not allowCorridor and not allowRooms and not allowCave:
			print("No tiles are allowed to have Enter/Exit objects")
			return

		# select set of tiles to use
		src = []
		if allowDeadends and not allowCorridor and not allowRooms:
			src += self.deadends
		elif not allowRooms and (allowDeadends or allowCorridor):
			src += self.corridors
		else:
			src += self.hostabletiles

		# clear source if needed
		if allowRooms and (not allowDeadends or not allowCorridor):
			if not allowDeadends:
				self._removeMatchTiles(src, self.deadends)
			if not allowCorridor:
				self._removeMatchTiles(src, self.corridors)
		...
		# load into overlays
		self._loadOverlay(rnd[0][0], rnd[0][1], OVERLAY_ENTER)
		self._loadOverlay(rnd[1][0], rnd[1][1], OVERLAY_EXIT)        

		# remove tiles from being used again
		if not allowOthers:
			self._removeMatchTiles(self.hostabletiles, rnd)

		return

	def placeRandomOverlays(self, overlay, count, hostType = None, setUnhostable = False):
		"""
		Places specified OVERLAY tiles randomly on hostable tiles (floor/corridor)
		This should only be called after the whole dungeon structure (floors, walls, ect.) is complete.

		Args:
		    <integer> overlay: this is the type of OVERLAY to be placed on tile
		    <integer> count: the count of items desired
		    <boolean> hostType: this is to specify rather item is only for room or corridor;
		                        if not specified then both are allowed
		...
		appCnt = 0
		rnd = []
		if len(self.hostabletiles)<count:          
			for c in range(count):
				if len(self.hostabletiles)<1:
					print("there are no hostable tiles available")
					return
				rnd = []
				if appCnt != count:
					rnd += self.hostabletiles
					for x,y,t in rnd:
						if appCnt<count and (t == hostType or hostType == None):
							appCnt += 1
							self._loadOverlay(x,y,overlay)
					if setUnhostable:
						self._removeMatchTiles(self.hostabletiles, rnd)
			return
		else:            
		...
		# remove items from hostabletiles array
		if setUnhostable:
			self._removeMatchTiles(self.hostabletiles, rnd)
	...
	def placeWater(self):
		"""
		Places water tiles around all wall, floor, door and corridor tiles that are empty.
		As some functions (like floodFill() and anything that uses it) dont distinguish between tile types it is best called later/last
