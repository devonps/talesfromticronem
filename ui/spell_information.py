...
def spell_pop_up(game_config, slot, gameworld, player):
    ...
    is_spell_on_cooldown = spellHelp.SpellUtilities.get_spell_cooldown_status(gameworld=gameworld, spell_entity=spell_entity)
    ...
    # spell name
    slot_spell_entity = spellHelp.SpellUtilities.get_spell_entity_from_spellbar_slot(gameworld=gameworld, slot=slot,
                                                                           player_entity=player)
    ...
    # spell description
    spell_description = spellHelp.SpellUtilities.get_spell_short_description(gameworld=gameworld, spell_entity=slot_spell_entity)
    terminal.printf(x=spell_popup_start_x + 1, y=top_of_popup + 1, s=spell_description)

    # spell type
    spell_type = spellHelp.SpellUtilities.get_spell_type(gameworld=gameworld, spell_entity=slot_spell_entity)
    terminal.printf(x=spell_popup_start_x + 1, y=top_of_popup + 2, s='Type:' + spell_type)

    # spell range
    spell_range = spellHelp.SpellUtilities.get_spell_max_range(gameworld=gameworld, spell_entity=slot_spell_entity)
    terminal.printf(x=spell_popup_start_x + 1, y=top_of_popup + 3, s='Range:' + str(spell_range))
    # spell cooldown
    spell_cooldown_value = spellHelp.SpellUtilities.get_spell_cooldown_time(gameworld=gameworld, spell_entity=slot_spell_entity)
    terminal.printf(x=spell_popup_start_x + 1, y=top_of_popup + 4, s='Cooldown turns:' + str(spell_cooldown_value))
    # spell status effects
    condi_effects = spellHelp.SpellUtilities.get_all_condis_for_spell(gameworld=gameworld, spell_entity=slot_spell_entity)
    boon_effects = spellHelp.SpellUtilities.get_all_boons_for_spell(gameworld=gameworld, spell_entity=slot_spell_entity)
    other_effects = spellHelp.SpellUtilities.get_all_resources_for_spell(gameworld=gameworld, spell_entity=slot_spell_entity)
    ...
    # spell direct damage
    equipped_weapons = mobileHelp.MobileUtilities.get_weapons_equipped(gameworld=gameworld, entity=player)
    caster_power = mobileHelp.MobileUtilities.get_mobile_primary_power(gameworld=gameworld, entity=player)
    spell_coeff = float(spellHelp.SpellUtilities.get_spell_damage_coeff(gameworld=gameworld, spell_entity=slot_spell_entity))

    weapon = which_weapon(equipped_weapons=equipped_weapons, slot=slot)

    weapon_strength = itemsHelp.ItemUtilities.calculate_weapon_strength(gameworld=gameworld, weapon=weapon)
    ...
    # spell AoE radius
    spell_use_aoe = spellHelp.SpellUtilities.get_spell_aoe_status(gameworld=gameworld, spell_entity=slot_spell_entity)
    if spell_use_aoe == 'True':
        aoe_radius = spellHelp.SpellUtilities.get_spell_aoe_size(gameworld=gameworld, spell_entity=slot_spell_entity)
    ...
    while player_not_pressed_a_key:
        ...
        if event_to_be_processed == 'keypress':
            ...
            if event_action == 0 and not is_spell_on_cooldown:
                # cast the spell
                player_not_pressed_a_key = False
                spellHelp.SpellUtilities.cast_spell(slot=slot, gameworld=gameworld, player=player)
