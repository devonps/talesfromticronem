...
class JewelleryUtilities:
    ...
    @staticmethod
    def create_jewellery_from_combat_kit(gameworld, gemstones, entity_id):
        gems = gemstones[0]
        pendent_gemstone = gems['pendent']
        left_ear_gemstone = gems['earring1']
        right_ear_gemstone = gems['earring2']
        left_ring_gemstone = gems['ring1']
        right_ring_gemstone = gems['ring2']

        # create jewellery entity
        pendant = JewelleryUtilities.create_jewellery_item(gameworld=gameworld, e_setting='copper', e_hook='copper',
                                                           e_activator=pendent_gemstone)
        left_ear = JewelleryUtilities.create_jewellery_item(gameworld=gameworld, e_setting='copper', e_hook='copper',
                                                            e_activator=left_ear_gemstone)

        right_ear = JewelleryUtilities.create_jewellery_item(gameworld=gameworld, e_setting='copper', e_hook='copper',
                                                             e_activator=right_ear_gemstone)

        left_ring = JewelleryUtilities.create_jewellery_item(gameworld=gameworld, e_setting='copper', e_hook='copper',
                                                             e_activator=left_ring_gemstone)

        right_ring = JewelleryUtilities.create_jewellery_item(gameworld=gameworld, e_setting='copper', e_hook='copper',
    ...
    def create_and_equip_jewellery_for_npc(gameworld, entity_id, jewellery_set, npc_class_file):
        ...
        for entityclass in class_file['classes']:
            if entityclass['name'] == entity_class:
                ...
                left_ear = JewelleryUtilities.create_jewellery_with_spell(gameworld=gameworld, bodylocation='earring1',
                                                                          e_setting='copper', e_hook='copper',
                                                                          e_activator=ear1_gemstone,
                                                                          playable_class=entity_class)

                right_ear = JewelleryUtilities.create_jewellery_with_spell(gameworld=gameworld, bodylocation='earring2',
                                                                           e_setting='copper', e_hook='copper',
    ...
    def get_list_of_spell_entities_for_equpped_jewellery(gameworld, player_entity):
        ...
        for a in ['neck', 'lear', 'rear', 'lhand', 'rhand']:
            entity_id = JewelleryUtilities.get_jewellery_entity_from_body_location(gameworld=gameworld,
                                                                                   entity=player_entity, bodylocation=a)

            if entity_id > 0:
                spell_id = JewelleryUtilities.get_spell_entity_from_jewellery(gameworld=gameworld,
                                                                              piece_of_jewellery=entity_id)
                if spell_id > 0:
    ...
    def add_jewellery_benefit(gameworld, entity, statbonus):
        ...
        if stat.lower() == 'condition damage':
            mobileHelp.MobileUtilities.set_mobile_secondary_condition_damage(gameworld=gameworld, entity=entity,
                                                                             value=benefit)
        ...
        if stat.lower() == 'healing power':
            mobileHelp.MobileUtilities.set_mobile_secondary_healing_power(gameworld=gameworld, entity=entity,
                                                                          value=benefit)
    ...
    def create_jewellery_for_utility_spells(gameworld, game_config, jewellery_set):
        ...
        for entityclass in class_file['classes']:
            if entityclass['name'] == entity_class:
                ...
                left_hand = JewelleryUtilities.create_jewellery_with_spell(gameworld=gameworld, bodylocation='ring1',
                                                                           e_setting='copper', e_hook='copper',
                                                                           e_activator=ring1_gemstone,
                                                                           playable_class=entity_class)

                right_hand = JewelleryUtilities.create_jewellery_with_spell(gameworld=gameworld, bodylocation='ring2',
                                                                            e_setting='copper', e_hook='copper',
                ...
                JewelleryUtilities.equip_jewellery(gameworld=gameworld, mobile=entity_id, bodylocation=neck,
                                                   trinket=pendant)
                JewelleryUtilities.equip_jewellery(gameworld=gameworld, mobile=entity_id, bodylocation=left_ear,
                                                   trinket=left_ear)
                JewelleryUtilities.equip_jewellery(gameworld=gameworld, mobile=entity_id, bodylocation=right_ear,
                                                   trinket=right_ear)

                JewelleryUtilities.equip_jewellery(gameworld=gameworld, mobile=entity_id, bodylocation=left_hand,
                                                   trinket=left_hand)

                JewelleryUtilities.equip_jewellery(gameworld=gameworld, mobile=entity_id, bodylocation=right_hand,
                                                   trinket=right_hand)

                # apply gemstone benefits
                jewellery_stat_bonus = JewelleryUtilities.get_jewellery_stat_bonus(gameworld=gameworld,
                                                                                   jewellery_entity=pendant)
                JewelleryUtilities.add_jewellery_benefit(gameworld=gameworld, entity=entity_id,
                ...
                jewellery_stat_bonus = JewelleryUtilities.get_jewellery_stat_bonus(gameworld=gameworld,
                                                                                   jewellery_entity=left_hand)
                JewelleryUtilities.add_jewellery_benefit(gameworld=gameworld, entity=entity_id,
                                                         statbonus=jewellery_stat_bonus)

                jewellery_stat_bonus = JewelleryUtilities.get_jewellery_stat_bonus(gameworld=gameworld,
                                                                                   jewellery_entity=right_hand)
                JewelleryUtilities.add_jewellery_benefit(gameworld=gameworld, entity=entity_id,
                                                         statbonus=jewellery_stat_bonus)
    ...
    def create_jewellery_item(gameworld, e_setting, e_hook, e_activator):
        ...
        for gemstone in gemstone_file['gemstones']:
            ...
            if file_gemstone == trinket_activator:
                ...
                itemsHelp.ItemUtilities.set_type_of_item(gameworld=gameworld, entity_id=piece_of_jewellery,
                                                         value='jewellery')
    ...
    @staticmethod
    def create_jewellery_with_spell(gameworld, bodylocation, e_setting, e_hook, e_activator, playable_class):
        """
        ...
        for gemstone in gemstone_file['gemstones']:
            ...
            if file_gemstone == trinket_activator:
                ...
                for spell_entity, (name, location, spell_type, item_type, spclass) in gameworld.get_components(
                        spells.Name,
                        spells.ItemLocation,
                        spells.SpellType,
                    ...
                    if spclass.label == playable_class and spell_type.label == 'utility' and item_type.label == 'jewellery' and location.label == bodylocation:
                        JewelleryUtilities.add_spell_to_jewellery(gameworld=gameworld,
                                                                  piece_of_jewellery=piece_of_jewellery,
                                                                  spell_entity=spell_entity)
                        JewelleryUtilities.earring_processing(bdl=bdl, trinket_activator=trinket_activator,
                                                              gemstone_string=gemstone_string, gameworld=gameworld,
                                                              piece_of_jewellery=piece_of_jewellery,
                                                              gemstone_attribute=gemstone['Attribute'],
                                                              gemstone_bonus=gemstone['Earring'])

                        JewelleryUtilities.pendant_processing(bdl=bdl, trinket_activator=trinket_activator,
                                                              gemstone_string=gemstone_string, gameworld=gameworld,
                                                              piece_of_jewellery=piece_of_jewellery,
                                                              gemstone_attribute=gemstone['Attribute'],
                                                              gemstone_bonus=gemstone['Amulet'])

                        JewelleryUtilities.hands_processing(bdl=bdl, trinket_activator=trinket_activator,
                                                            gemstone_string=gemstone_string, gameworld=gameworld,
                                                            piece_of_jewellery=piece_of_jewellery,
                                                            gemstone_attribute=gemstone['Attribute'],
                                                            gemstone_bonus=gemstone['Ring'])
    ...
    @staticmethod
    def hands_processing(bdl, trinket_activator, gemstone_string, gameworld, piece_of_jewellery, gemstone_attribute,
                         gemstone_bonus):
        if 'hands' in bdl:
            # create a ring
            ...
            JewelleryUtilities.common_jewellery_create_method(gameworld=gameworld,
                                                              piece_of_jewellery=piece_of_jewellery, nm=nm, desc=desc,
                                                              trinket_activator=trinket_activator)

    @staticmethod
    def pendant_processing(bdl, trinket_activator, gemstone_string, gameworld, piece_of_jewellery, gemstone_attribute,
                           gemstone_bonus):
        if 'neck' in bdl:
            # create a pendant
    ...
    @staticmethod
    def earring_processing(bdl, trinket_activator, gemstone_string, gameworld, piece_of_jewellery, gemstone_attribute,
                           gemstone_bonus):
        if 'ear' in bdl:
            # create an earring
